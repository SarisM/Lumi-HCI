...
export function NutritionScreen() {
  const { nutritionalNeeds, mealIntakes, updateMealIntake, getTotalIntake } = useUser();
  ...
  const updateProtein = (meal: "breakfast" | "lunch" | "dinner", value: number) => {
    updateMealIntake(meal, { ...mealIntakes[meal], protein: value });
  };

  const updateFiber = (meal: "breakfast" | "lunch" | "dinner", value: number) => {
    updateMealIntake(meal, { ...mealIntakes[meal], fiber: value });
  };

  return (
    <div className="relative h-full bg-gradient-to-br from-pink-50 via-green-50 to-yellow-50 overflow-hidden p-6 flex flex-col">
      {/* Background elements */}
      <motion.div
